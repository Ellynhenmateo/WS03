...
<?= loadPartial('navbar'); ?>
<?php
$user = (object) ($user ?? $users ?? []);
$old = fn($field) => htmlspecialchars($user->{$field} ?? '', ENT_QUOTES, 'UTF-8');
?>
        ...
        <form method="POST" action="/auth/register">
            <div class="mb-4">
                <input type="text" name="name" placeholder="Full Name"
                    class="w-full px-4 py-2 border rounded focus:outline-none" value="<?= $old('name') ?>" />
            </div>
            <div class="mb-4">
